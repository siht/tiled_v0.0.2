...
## author: Jos� Carlos Tzompantzi de Jes�s
## license: LGPL v3
## si no recibiste copia de la licencia ir a http://www.gnu.org/licenses/lgpl.html
## queda a disposici�n de quien quiera ocuparla y modificarla
## no hay garant�a de que funcione, ni me hago responsable del uso potencial
## de este c�digo
...
def metodo(name, wrap='wrap'):
    # esta funci�n es externa, ya que no se me permite incluirla en la definici�n de una metaclase
    # adem�s de que el truco con funciones lambda no me funcion�
...
class MetaWrap(type):
    ...
    '''
    # m�todos que no quiero sobreescribir
    ...
    def __call__(cls, *params, **kwparams):
        # obtener un par�metro o None si no hay
        wrap = None
        if params:
            wrap = params[0]
        ...
        for attr in obj_attrs:
            # verificar que no est� sobre escrito
            if attr not in cls._clss[cls]:
                if callable(wrap.__getattribute__(attr)):
                    # asignar m�todos a la clase y hacer que los m�todo tengan un valor por default
        ...
        # y a�adirle al vuelo el objeto a envolver
        obj.wrap = wrap
        return obj
...
if __name__ == '__main__':
    ...
    # class DecoSimple(Decorador):
        ...
        # def mthod(self): # y ya puede 'sobre escribir' un m�todo
